...
export function exportAsText(state) {
  ...
  if (state.matchups && state.matchups.length) {
    for (const matchup of state.matchups) {
      const ins = [];
      const outs = [];
      for (const [cardName, rawPlan] of Object.entries(state.plan || {})) {
        const entry = rawPlan && rawPlan[matchup];
        if (!entry) continue;
        // Support both legacy string form and new { dir, count } form.
        const dir = typeof entry === "string" ? entry : entry.dir;
        const count = typeof entry === "string" ? null : entry.count;
        const label = count && count > 0 ? (count + "x " + cardName) : cardName;
        if (dir === "in") ins.push(label);
        else if (dir === "out") outs.push(label);
      }
